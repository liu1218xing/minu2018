...
$sql="select p.id,p.pName,p.pSn,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName,p.cId,aImg from product as p join mind_cate c on p.cId=c.id where p.cId={$cid} order by p.sortid desc,p.pubtime desc limit {$offset},{$pageSize}";
$pros=fetchAll($sql);
// var_dump($_SERVER);
// $id=$_REQUEST['id'];
...
<body>
  ...
  <div class="navbar navbar-default  bs-docs-nav minu-navbar hidden-xs" id="mainnav" role="navigation">
    ...
    <!-- banner 图片 -->
    <div class="container-fluid">
      <div class="row minupro homepage-panel minu-product" id="minucase">
        <img src="/images/minu-product.jpg">
      </div>
    </div>
        ...
        <div class="block">
          <header>
            <div class="hot-pro-title">
                <h2 class="text-h2"><?php echo $crow['cName'] ; ?></h2>
                <!-- <hr> -->
            </div>
            ...
            <div class="row hot-pro-list">
              ...
              <div class="col-sm-4 col-xs-6 hot-pro">
                <div class="hot-pro-pic product-img">
                  <a href="/productDetails/<?php echo $pro['id'];?>.html" target="_blank">
                    <img height="204" width ="165" class="fill avatar" src="/admin/uploads/<?php echo $pro['aImg'];?>"></a>
                </div>
                <div class="hot-title">
                  <h2>
                    <a href="/productDetails/<?php echo $pro['id'];?>.html" target="_blank"><?php echo $pro['pName']; ?></a>
                  </h2>
                </div>
              </div>
              <?php endforeach ?>  
